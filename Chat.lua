-- Pro Chat v13 - Optimized & Clean (Fixed)
...
local MAX_MESSAGES = 150 -- سقف عدد الرسائل المعروضة (يمنع تراكم العناصر بالذاكرة)

local userColors = {}
local playerBubbles = {}
local lastId = 0          -- آخر id شفناه، بديل عن shownIds اللي كانت تكبر بلا نهاية
local messageQueue = {}   -- طابور لتتبع أقدم رسالة معروضة لحذفها عند تجاوز السقف
...
-- تثبيت السكرول بالأسفل بشكل موثوق (بديل عن task.defer + AbsoluteCanvasSize)
...
local function addMessage(user, msg)
    ...
    -- تتبع الرسائل وحذف الأقدم لو تجاوزنا السقف (يمنع تراكم عناصر GUI بلا نهاية)
...
local function getHead(char)
    ...
    -- نفضل الرأس، بعدها HumanoidRootPart (مركز الجسم)، وبعدها أي بارت كحل أخير
...
--         استقبال الرسائل
-- ══════════════════════════════════════
-- بدل ما نسحب آخر 40 رسالة بترتيب تصاعدي (وهذا كان يجيب نفس أول 40 رسالة
-- للأبد ويوقف استقبال أي رسالة جديدة بعدها)، نسوي تحميل أولي للتاريخ،
-- وبعدها نسحب بس الرسائل الجديدة (id أكبر من آخر id شفناه).

task.spawn(function()
    -- تحميل أولي: آخر 40 رسالة (الأحدث)، تُعرض كتاريخ بدون فقاعات
    pcall(function()
        local res = request({
            Url = API_URL .. "?select=*&order=id.desc&limit=40",
            Method = "GET",
            Headers = HEADERS
        })
        ...
        if res and res.Body then
            ...
            if type(data) == "table" then
                -- نعكس الترتيب لأنها جاية من الأحدث للأقدم
                for i = #data, 1, -1 do
                    local v = data[i]
                    if v.id then
    ...
    -- بعد التحميل الأولي: نسحب فقط الرسائل الأحدث من lastId
    while task.wait(0.4) do
        pcall(function()
            local res = request({
                Url = API_URL .. "?select=*&order=id.asc&limit=50&id=gt." .. lastId,
                Method = "GET",
                Headers = HEADERS
            })
            ...
            for _, v in ipairs(data) do
                if v.id and v.id > lastId then
                    ...
                    -- فقاعة فقط للاعبين في روبلوكس (مو ديسكورد، ومو رسالتنا)
...
-- تنظيف عند خروج اللاعب (كانت ناقصة) - يمنع تراكم بيانات لاعبين غادروا
