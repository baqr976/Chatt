...
local function createBubble(container, message)
    ...
    bubble.BackgroundColor3 = Color3.fromRGB(248, 249, 255)
    bubble.BackgroundTransparency = 0.04
    bubble.BorderSizePixel = 0
    Instance.new("UICorner", bubble).CornerRadius = UDim.new(0, 15)
    ...
    local bubbleGradient = Instance.new("UIGradient", bubble)
    bubbleGradient.Rotation = 90
    bubbleGradient.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, Color3.fromRGB(255,255,255)),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(232,235,255))
    })
...
local function sendMessage(text)
    ...
    if #username > 32 then username = string.sub(username, 1, 32) end

    showBubbleAbovePlayer(username, text)
    local data = {username = username, message = text}
    task.spawn(function()
        pcall(function()
            request({
...
local uiHeartbeatConnection
local lastViewport = Vector2.zero
local function fastUIRefresh()
    local cam = workspace.CurrentCamera
    if not cam then return end
    local v = cam.ViewportSize
    if v ~= lastViewport then
        lastViewport = v
...
if RunService and RunService.RenderStepped then
    uiHeartbeatConnection = RunService.RenderStepped:Connect(fastUIRefresh)
end
...
print("✅ CHATTTT V6 — UI polished, Supabase schema matched, network preserved")
